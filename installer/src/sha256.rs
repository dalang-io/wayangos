//! SHA-256 and base64 — just enough for OpenSSH key fingerprints
//! (`SHA256:` + unpadded base64 of the digest of the decoded key blob).
...
pub fn base64_decode(s: &str) -> Option<Vec<u8>> {
    ...
    for c in s.trim_end_matches('=').bytes() {
        ...
        // only the low `bits` (< 14) are pending; keep acc from overflowing
...
mod tests {
    ...
    fn base64_roundtrip() {
        for s in [&b""[..], b"f", b"fo", b"foo", b"foob", b"fooba", b"foobar"] {
            assert_eq!(base64_decode(&base64_nopad(s)).unwrap(), s);
        }
        assert_eq!(base64_nopad(b"foobar"), "Zm9vYmFy");
        assert_eq!(base64_nopad(b"fo"), "Zm8");
        assert_eq!(base64_decode("Zm8=").unwrap(), b"fo");
        assert!(base64_decode("Zm*8").is_none());
    }
}
